...
class RoomTest_Random {

	@Test
	void testEnemies() {
		int numTests = 100;
		Room r[] = new Room[numTests];
		...
		for(int i = 0; i < numTests; i++) {
			enemy[i] = r[i].getRoomResources().getEnemy();
		}
	...
	void testPotions() {
		...
		for(int i = 0; i < numTests; i++) {
			potions[i] = r[i].getRoomResources().getPotion();
		}
	...
	void testWeapons() {
		...
		for(int i = 0; i < numTests; i++) {
			weapon[i] = r[i].getRoomResources().getWeapon();
		}
