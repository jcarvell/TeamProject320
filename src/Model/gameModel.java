...
public class gameModel {
	...
	////////////////////////////////////////////////////////////////////////////////////////////////
	private static int maxPotionHealth = 10, maxPotionSpeed = 10; // I choose these value arbitrarily, feel free to
																	// change them
	...
	String[] potionNames = { "Witch's Brew", "Slug Trouble", "Spooky Scary Potion", "Green Vial", "Blue Vial",
			"Red potion", "Orange Juice" };
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////
	/*
	 * public NPCs(String roomName) { this.roomName = roomName; }
	 */
	...
	public int printNPCinteraction(User user) {
		...
		if (roomName == "Meadow") {
			System.out.println(
					"There is a tree in the middle of the meadow with a man standing at the base of the tree.");
	...
	public void Potion() {
		...
		healthIncreaseAmount = rand.nextInt(maxPotionHealth) + 1; // set the health increase for the potion to a random
																	// number between one and maxPotionHealth
		speedIncreaseAmount = rand.nextInt(maxPotionSpeed) + 1; // set the speed increase for the potion to a random
																// number between one and maxPotionSpeed
		potion_name = potionNames[rand.nextInt(7)]; // set the name of the potion to a name in the potionNames array,
													// randomly choosing index 0 through 7
	}
	...
	// Added this constructor so that the Room() constructor in Game wouldn't have
	// an error
	public void Room() {
		Random rand = new Random(); // instance of random class
	...
	public void RoomResources() {
		Random rand = new Random(); // instance of random class
		numEnemies = rand.nextInt(maxEnemies + 1); // set the number of enemies to a random number between zero and
													// maxEnemies
		...
		numPotions = rand.nextInt(maxPotions + 1); // set the number of potions to a random number between zero and
													// maxPotions
		numWeapons = rand.nextInt(maxWeapons + 1); // set the number of weapons to a random number between zero and
													// maxWeapons
		...
		if (npcChoice == 1) {
			...
			if (resourceChoice == 1 && getWeapon().getName() != "No Weapon") {
				if (getWeapon().getStrengthBuff() < user.getStrength()) {
					...
					System.out.println("Your current strength is " + user.getStrength() + " and the "
							+ getWeapon().getName() + "only has " + getWeapon().getStrengthBuff());
	...
	public void Weapon() {
		...
		// rand.nextInt(n) gets a random number between 0(inclusive) and the number
		// passed in this argument(n), exclusive
		int random1 = rand.nextInt(upperbound); // random number between 1 and 5, so it excludes "unarmed" as a weapon
												// name
