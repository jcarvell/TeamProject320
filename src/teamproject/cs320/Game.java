		...
		GAME: while (running) {
			...
			while (user.getHealth() > 0 && choice1.getRoomResources().getEnemy().getHealth() <= 0) {
				...
				if (save == 1) {
					...
					String i = db.insertPlayer(playerName, user.getHealth(), user.getSpeed(), user.getStrength(),
							user.currentWeaponName(), user.getStrength(), "health", 10, 10, choice1.getName(),
				...
				System.out.println(
						"You have two options 1. " + choice1.getName() + " or you can choose 2. " + choice2.getName());
			...
			while (choice1.getRoomResources().getEnemy().getHealth() > 0) {
				...
				if (input == 1) {
					...
					int damageDone = rand.nextInt(user.getStrength()); // changed maxWeaponDamage[] to maxWeaponDamage
																		// and declared it above
					...
					if (choice1.getRoomResources().getnumPotions() > 0) {
						user.setHealth(
								user.getHealth() + choice1.getRoomResources().getPotion().getHealthIncreaseAmount()); // potionHealAmount
																														// was
																														// never
																														// declared
																														// so
						...
						System.out.println("Your new Health is " + user.getHealth() + " you now have "
			...
			System.out.println("You currently have " + user.getHealth() + " health and "
					+ choice1.getRoomResources().getnumPotions() + " potions.");
